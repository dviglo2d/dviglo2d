...
void App::draw()
{
    ...
    // Рисуем карту
    fs::path base_path = get_base_path();
    shared_ptr<Texture> tilemap_flat_png = DV_TEXTURE_CACHE->get(base_path / "village_data/tilemap_flat.png");
    ...
    for (i32 y = 0; y < map_->size().y; ++y)
    {
        for (i32 x = 0; x < map_->size().x; ++x)
        {
            ...
            sprite_batch_->draw_sprite(tilemap_flat_png.get(), tile_pos, &tile.uv);
        }
    }
    ...
    shared_ptr<Texture> tilemap_elevation_png = DV_TEXTURE_CACHE->get(base_path / "village_data/tilemap_elevation.png");
    for (i32 y = 0; y < map_->size().y && y <= player_tile_y; ++y)
    {
        for (i32 x = 0; x < map_->size().x; ++x)
        {
            ...
            if (tile.obstacle)
            {
                ...
                sprite_batch_->draw_sprite(tilemap_elevation_png.get(), sprite_pos, &tile.obstacle_uv);
            }
        }
    }

    // Рисуем персонажа
    shared_ptr<Texture> warrior_blue_png = DV_TEXTURE_CACHE->get(base_path / "village_data/warrior_blue.png");
    Rect player_uv(0.f, 0.f, 192.f, 192.f);
    vec2 player_sprite_pos(player_->pos.x - 96.f, player_->pos.y - 128.f); // На спрайте много пустого места
    sprite_batch_->draw_sprite(warrior_blue_png.get(), player_sprite_pos, &player_uv);

    // Рисуем коллайдер персонажа
    sprite_batch_->set_shape_color(0x90FFFFFF);
    sprite_batch_->draw_rect(Rect(player_->pos - static_cast<vec2>(player_->half_size), static_cast<vec2>(player_->half_size * 2)));
