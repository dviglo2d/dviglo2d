...
void App::start()
{
    ...
    DV_LOG->writef_info("Папка программы: {}", base_path.string());
    ...
    music_ = Mix_LoadMUS(music_path.string().c_str());

    if (!music_)
        DV_LOG->writef_error("{} | !music_ | {}", DV_FUNC_SIG, SDL_GetError());

    if (!Mix_PlayMusic(music_, -1))
        DV_LOG->writef_error("{} | !Mix_PlayMusic(...) | {}", DV_FUNC_SIG, SDL_GetError());
