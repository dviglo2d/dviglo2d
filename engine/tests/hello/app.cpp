...
void App::setup()
{
    engine_params::log_path = get_pref_path("dviglo2d", "tests") + "hello.log";
...
void App::start()
{
    ...
    DV_LOG->writef_info("Командная строка: {}", join(args(), " "));
    DV_LOG->writef_info("Папка программы: {}", base_path);
...
#if 0
    StrUtf8 font_path = base_path + "engine_test_data/fonts/ubuntu-r_20_simple.fnt";
    ...
    if (!music_)
        DV_LOG->writef_error("App::start(): !music_ | {}", SDL_GetError());

    if (!Mix_PlayMusic(music_, -1))
        DV_LOG->writef_error("App::start(): !Mix_PlayMusic(...) | {}", SDL_GetError());
