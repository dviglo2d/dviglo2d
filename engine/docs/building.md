...
1. GCC 13+
2. Clang 16+. Clang использует библиотеки от GCC (libstdc++),
   поэтому GCC 13 всё равно должен быть установлен

GCC 11 не поддерживает `shared_ptr<T[]>`. GCC 12 не поддерживает `std::format`.

Clang 15 не компилирует код:
...
```
# Установка OpenMP для Clang 16
sudo apt install libomp-16-dev
```

Это добавит в зависимости пакет libomp5-16t64, который должен быть установлен
