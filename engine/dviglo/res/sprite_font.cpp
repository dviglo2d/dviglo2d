...
SpriteFont::SpriteFont(const fs::path& path)
{
    ...
    {
        DV_LOG->writef_error(R"({} | {} | root_node.name() != string("font"))", DV_FUNC_SIG, path);
        return;
    }
    ...
    if (!atlases_node)
    {
        DV_LOG->writef_error("{} | {} | !atlases_node", DV_FUNC_SIG, path);
        return;
    }
    ...
    for (i32 i = 0; i < num_atlases; ++i)
    {
        if (!atlas_node)
        {
            DV_LOG->writef_error("{} | {} | !atlas_node", DV_FUNC_SIG, path);
            return;
        }
...
void SpriteFont::save(const fs::path& path) const
{
    ...
    for (const shared_ptr<Texture>& texture : textures_)
    {
        if (!texture->image())
        {
            DV_LOG->writef_error("{} | {} | !texture->image()", DV_FUNC_SIG, path);
            return;
        }
    }
    ...
    if (ext != "fnt")
    {
        DV_LOG->writef_error(R"({} | {} | ext != "fnt")", DV_FUNC_SIG, path);
        return;
    }
