...
Image::Image(ivec2 size, i32 num_components, u32 color, bool warning)
{
    ...
    if (!num_bytes)
    {
        ...
        if (warning)
            DV_LOG->writef_warning("{} | !num_bytes", DV_FUNC_SIG);
    }
    else
    {
        size_ = size;
...
Image::Image(const fs::path& path, bool use_error_image)
{
    ...
    if (!data_)
    {
        DV_LOG->writef_error("{} | {} | {}", DV_FUNC_SIG, path, stbi_failure_reason());
...
void Image::save_png(const fs::path& path) const
{
    ...
    if (!png_data)
    {
        DV_LOG->writef_error("{} | {} | !png_data", DV_FUNC_SIG, path);
    }
    else
    {
    ...
    DV_LOG->writef_info("{} | {} | Saved in {} ms", DV_FUNC_SIG, path, duration_ms);
}
...
void Image::paste(const Image& img, ivec2 pos)
{
    ...
    if (img.num_components() != num_components())
    {
        ...
        DV_LOG->writef_error("{} | img.num_components() != num_components()", DV_FUNC_SIG);
        return;
    }
...
Image Image::to_rgba(u32 color)
{
    ...
    if (num_components() != 1)
    {
        DV_LOG->writef_error("{} | num_components() != 1", DV_FUNC_SIG);
        return *this;
    }
