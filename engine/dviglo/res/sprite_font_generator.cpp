...
// Отправляет атлас в память видеокарты, а также добавляет в TextureCache 
static shared_ptr<Texture> to_cached_texture(shared_ptr<Image> atlas)
{
    assert(atlas->num_components() == 4);

    shared_ptr<Texture> ret = make_shared<Texture>(atlas, true);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

    DV_TEXTURE_CACHE->add(ret);
...
static i32 calc_line_height(const FT_Face face, const f32 addend = 0.f)
{
    ...
    if (addend == 0.f)
        ...
        return round_to_i32(to_f32(height) + addend);
}
...
SpriteFont::SpriteFont(const SFSettingsSimple& settings, i64* generation_time_ms)
{
    ...
    while (glyph_index != 0)
    {
        ...
        // Забираем отрендеренный grayscale глиф из слота
        RenderedGlyph rendered_glyph(char_code, face);

        // Размываем grayscale глиф
        rendered_glyph.blur(settings.blur_radius);

        // Добавляем grayscale глиф в список для будущей упаковки в атласы
    ...
    // Упаковываем grayscale глифы в атласы
    ...
    {
        // Преобразуем grayscale атлас в цветной RGBA
        shared_ptr<Image> colored_atlas = make_shared<Image>(atlas->to_rgba(settings.color));

        // Отправляем атлас в память GPU, а также добавляем в TextureCache и список текстур
        textures_.push_back(to_cached_texture(colored_atlas));
    }
...
SpriteFont::SpriteFont(const SFSettingsContour& settings, i64* generation_time_ms)
{
    ...
    while (glyph_index != 0)
    {
        ...
        RenderedGlyph rendered_glyph = render_glyph_contour(face.get(), settings); // Grayscale
...
SpriteFont::SpriteFont(const SFSettingsOutlined& settings, i64* generation_time_ms)
{
    ...
    while (glyph_index != 0)
    {
        ...
        FT_Load_Glyph(face.get(), glyph_index, load_flags);
        RenderedGlyph rendered_glyph = render_glyph_outlined(face.get(), settings); // RGBA
        rendered_glyph.code_point = char_code;
        glyph_packer.add(std::move(rendered_glyph));
    ...
    // Атласы уже преобразованы в RGBA
    for (shared_ptr<Image> atlas : atlases)
        textures_.push_back(to_cached_texture(atlas));
