...
private:
    ...
    void done()
    {
        if (face_)
        {
            ...
            if (error)
                DV_LOG->writef_error("{} | FT_Done_Face(...) error {}", DV_FUNC_SIG, error);
...
public:
    FreeTypeFace(const SFSettings& settings)
    {
        ...
        if (data.empty())
        {
            DV_LOG->writef_error("{} | data.empty()", DV_FUNC_SIG);
            return;
        }
        ...
        if (error)
        {
            DV_LOG->writef_error("{} | FT_New_Memory_Face(...) error {}", DV_FUNC_SIG, error);
            done();
            return;
        }

        if (face_->num_faces != 1)
            DV_LOG->writef_warning("{} | face_->num_faces != 1 | {}", DV_FUNC_SIG, face_->num_faces);

        error = FT_Select_Charmap(face_, FT_ENCODING_UNICODE);
        if (error)
        {
            DV_LOG->writef_error("{} | FT_Select_Charmap(...) error {}", DV_FUNC_SIG, error);
            done();
            return;
        }
        ...
        if (error)
        {
            DV_LOG->writef_error("{} | FT_Set_Pixel_Sizes(...) error {}", DV_FUNC_SIG, error);
            done();
            return;
        }
    }
...
public:
                  ...
                  const FT_UInt glyph_index, const FT_ULong char_code)
    {
        ...
        if (error)
        {
            DV_LOG->writef_error("{} | FT_Load_Glyph(...) | {}", DV_FUNC_SIG, error);
            error_image();
            return;
        }
                  ...
                  const FT_UInt glyph_index, const FT_ULong char_code)
    {
        ...
        if (error)
        {
            DV_LOG->writef_error("{} | FT_Get_Glyph(...) error | {}", DV_FUNC_SIG, error);
            error_image();
            return;
        }
        ...
        if (error)
        {
            DV_LOG->writef_error("{} | FT_Stroker_New(...) error | {}", DV_FUNC_SIG, error);
            error_image();
            return;
        }
        ...
        if (error)
        {
            DV_LOG->writef_error("{} | FT_Glyph_Stroke(...) error | {}", DV_FUNC_SIG, error);
            error_image();
            return;
        }
        ...
        if (error)
        {
            DV_LOG->writef_error("{} | FT_Glyph_To_Bitmap(...) error | {}", DV_FUNC_SIG, error);
            error_image();
            return;
        }
                  ...
                  const FT_UInt glyph_index, const FT_ULong char_code)
    {
        ...
        if (error)
        {
            DV_LOG->writef_error("{} | Inflated FT_Glyph_To_Bitmap(...) error | {}", DV_FUNC_SIG, error);
            error_image();
            return;
        }
        ...
        if (bitmap_glyph->bitmap.width && bitmap_glyph->bitmap.rows) // Если глиф не пустой
        {
            ...
            if (error)
            {
                DV_LOG->writef_error("{} | Inner FT_Glyph_To_Bitmap(...) error | {}", DV_FUNC_SIG, error);
                error_image();
                return;
            }
...
SpriteFont::SpriteFont(const T& settings, i64* generation_time_ms)
{
    ...
    DV_LOG->writef_debug("{} | {} | Generated in {} ms", DV_FUNC_SIG, settings.src_path, duration_ms);
}
