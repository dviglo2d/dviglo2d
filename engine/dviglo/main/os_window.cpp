...
OsWindow::OsWindow()
{
    ...
    {
        DV_LOG->writef_error("{} | !SDL_InitSubSystem(SDL_INIT_VIDEO) | {}", DV_FUNC_SIG, SDL_GetError());
        return;
    }
    ...
    if (!window_)
    {
        DV_LOG->writef_error("{} | !window_ | {}", DV_FUNC_SIG, SDL_GetError());
        return;
    }
    ...
    if (!gl_context_)
    {
        DV_LOG->writef_error("{} | !gl_context_ | {}", DV_FUNC_SIG, SDL_GetError());
        return;
    }
    ...
    {
        DV_LOG->writef_debug("{} | vsync_ret < 0 && engine_params::vsync < 0 | {}", DV_FUNC_SIG, SDL_GetError());
    ...
    if (vsync_ret < 0)
        DV_LOG->writef_error("{} | vsync_ret < 0 | {}", DV_FUNC_SIG, SDL_GetError());
    ...
    if (!gl_version)
    {
        DV_LOG->writef_error("{} | !gl_version", DV_FUNC_SIG);
        return;
    }
