...
// Перенести сюда остальные подсистемы из application_base.hpp
#include "../graphics/sampler_cache.hpp"
...
// TODO: МОжет переименвоать в EngineSubsystems?
struct MainSubsystems
{
    ...
    SDL_AppResult init(i32 argc, char* argv[])
    {
        ...
        bool ret_success;
        sampler_cache = std::make_unique<SamplerCache>(ret_success);
        if (!ret_success)
            return SDL_APP_FAILURE; // Сообщение об ошибке уже выведено
