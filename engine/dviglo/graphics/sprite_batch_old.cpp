...
SpriteBatchOld::SpriteBatchOld()
{
    ...
    t_shader_program_ = DV_SHADER_CACHE_OLD->get(base_path / "engine_data/shaders/vert_color.vert", base_path / "engine_data/shaders/vert_color.frag");
    q_current_shader_program_ = q_default_shader_program_ = DV_SHADER_CACHE_OLD->get(base_path / "engine_data/shaders/vert_color_texture.vert", base_path / "engine_data/shaders/vert_color_texture.frag");
    ...
    f32 rotation, vec2 origin, vec2 scale, FlipModes flip_modes)
{
    ...
    {
        const SFGlyph& glyph = font->get_glyph(unicode_text[i]);
        Rect rect(glyph.rect);
        vec2 offset(glyph.offset);

        sprite.texture = font->textures()[glyph.atlas_index].get();
...
vec2 SpriteBatchOld::measure_string(const StrUtf8& text, SpriteFont* font, vec2 scale)
{
    ...
    for (c32 code_point : unicode_text)
    {
        const SFGlyph& glyph = font->get_glyph(code_point);
        width += glyph.advance_x * scale.x;
    }

    return vec2(width, font->line_height() * scale.y);
}
