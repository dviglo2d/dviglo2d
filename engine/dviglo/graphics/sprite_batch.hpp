...
public:
    ...
    Rect measure_string_sprites(const StrUtf8& text, SpriteFont* font, glm::vec2 position = {0.f, 0.f},
        f32 rotation = 0.0f, glm::vec2 origin = {0.f, 0.f}, glm::vec2 scale = {1.f, 1.f}, FlipModes flip_modes = FlipModes::none);

    // Ширина и высота неповёрнутой строки.
    // Для расчётов используются данные из шрифта (advance_x и line_height), а не реальные размеры спрайтов глифов.
    // Поэтому данную функцию можно использовать, чтобы в конец одной строки дорисовать другую строку
    glm::vec2 measure_string(const StrUtf8& text, SpriteFont* font, glm::vec2 scale = {1.f, 1.f});
};
