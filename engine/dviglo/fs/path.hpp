...
// Заменяет '\\' на '/'
constexpr StrUtf8 to_internal(const StrViewUtf8 path)
{
...
constexpr std::wstring to_win_native(const StrViewUtf8 path)
{
...
constexpr StrUtf8 trim_end_slash(const StrViewUtf8 path)
{
    StrUtf8 ret(path);
...
constexpr void split_path(const StrViewUtf8 path, StrUtf8* out_dir_path = nullptr,
...
constexpr StrUtf8 get_file_name(const StrViewUtf8 path)
{
    StrUtf8 ret;
    split_path(path, nullptr, &ret);
    return ret;
}
