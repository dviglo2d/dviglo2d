...
// TeeBuffer перехватывает только std::cout и std::cerr, но не printf(...) и fprintf(stderr, ...)
#define DV_USE_TEE_BUFFER 1
...
// Пока существует объект, вывод в консоль будет дублироваться в файл
class Log
{
private:
...
public:
    ...
    // Функции ниже могут быть static, и тогда в лог можно будет писать так: Log::write*(...).
    // Но пусть будет так, вдруг на каких-то платформах невозможно перехватить stdout и stderr
