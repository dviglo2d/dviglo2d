...
// Если подменить std::cout.rdbuf(свой_разветвитель) и std::cerr.rdbuf(свой_разветвитель),
// то это не повлияет на printf(...) и fprintf(stderr, ...), которые используются в сторонних библиотеках.
// Поэтому подменяем stderr и stdout
