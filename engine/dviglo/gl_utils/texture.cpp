...
static TextureParams try_load_xml(const fs::path& path)
{
    ...
    if (!result)
    {
        DV_LOG->writef_error("{} | {} | !result", DV_FUNC_SIG, path);
        return ret;
    }
    ...
    {
        DV_LOG->writef_error(R"({} | {} | root_node.name() != string("texture"))", DV_FUNC_SIG, path);
        return ret;
    }
    ...
    for (xml_node child : root_node)
    {
        ...
        if (key == "GL_TEXTURE_MIN_FILTER")
        {
            ...
            if (value == "GL_NEAREST")
                ...
                DV_LOG->writef_error("{} | {} | GL_TEXTURE_MIN_FILTER | incorrect value | {}", DV_FUNC_SIG, path, value);
        }
        else if (key == "GL_TEXTURE_MAG_FILTER")
        {
            ...
            if (value == "GL_NEAREST")
                ...
                DV_LOG->writef_error("{} | {} | GL_TEXTURE_MAG_FILTER | incorrect value | {}", DV_FUNC_SIG, path, value);
        }
        else
        {
            DV_LOG->writef_error("{} | {} | incorrect key | {}", DV_FUNC_SIG, path, key);
        }
    }
...
Texture::Texture(const fs::path& path)
{
    ...
    {
        DV_LOG->writef_error("{} | image->num_components() == {}", DV_FUNC_SIG, image->num_components());
        from_error_image(4);
        return;
    }
