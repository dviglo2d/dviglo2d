...
Texture::Texture(const StrUtf8& file_path)
{
    ...
    {
        DV_LOG->writef_error("{} | image->num_components() == {}", DV_FUNCSIG, image->num_components());
        from_error_image(4);
        return;
    }
    ...
    , num_components_(num_components)
{
    ...
    GLint internal_format = to_interal_format(num_components);
    GLenum image_format = to_image_format(num_components);
...
void Texture::from_error_image(i32 num_component)
{
    ...
    glGenTextures(1, &gpu_object_name_);
    glBindTexture(GL_TEXTURE_2D, gpu_object_name_);
    GLint internal_format = to_interal_format(error_image.num_components());
    GLenum image_format = to_image_format(error_image.num_components());
