...
Texture::Texture(const Image& image)
{
    ...
    {
        DV_LOG->write_error(format("Texture::Texture(const Image&): image.num_components() == {}", image.num_components()));

        img_data = error_image.data();

        if (error_image.num_components() == 3)
            img_format = GL_RGB;
        else
            img_format = GL_RGBA;
    ...
    glGenTextures(1, &gpu_object_name_);
    glBindTexture(GL_TEXTURE_2D, gpu_object_name_);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, size_.x, size_.y, 0, img_format, GL_UNSIGNED_BYTE, img_data);
    glGenerateMipmap(GL_TEXTURE_2D);
}
