...
struct VulkanImage
{
    ...
    // Индекс текстуры в огромном массиве всех текстур (dstArrayElement в descriptor set в DV_TEXTURE_SET).
    // Если 0xFFFFFFFF, значит текстура не в DV_TEXTURE_SET
    u32 index = 0xFFFFFFFF;

    static std::optional<VulkanImage> create(glm::uvec2 size, vk::ImageUsageFlags usage, vk::Format format = vk::Format::eR8G8B8A8Unorm);
    static std::optional<VulkanImage> load(const fs::path& path, vk::Format format = vk::Format::eR8G8B8A8Unorm);
