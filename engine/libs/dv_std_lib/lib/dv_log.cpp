    ...
    : file_stream_(path)
{
    ...
    write_debug("Log constructed"); // Это сообщение попадёт только в консоль
...
Log::~Log()
{
    ...
    writef_debug("Subsystem {} destructed | {}", index(), DV_FUNC_SIG); // Это сообщение попадёт только в консоль
    instance_ = nullptr;
}
